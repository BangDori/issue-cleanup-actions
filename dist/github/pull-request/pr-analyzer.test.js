...
// src/github/pull-request/pr-analyzer.test.ts
describe("pr-analyzer", () => {
  ...
  describe("getLinkedPRs", () => {
    it("should return linked PRs from issue timeline", async () => {
      ...
      mockOctokit.rest.issues.listEventsForTimeline = vi.fn().mockResolvedValue({ data: mockTimeline });
      ...
      globalExpect(result[0].source.issue.number).toBe(10);
      globalExpect(result[1].source.issue.number).toBe(11);
    });
    it("should filter out non-PR cross-references", async () => {
      const mockTimeline = [
        ...
        {
          event: "cross-referenced",
          source: { type: "issue", issue: { number: 11 } }
          // No pull_request
        },
        {
          event: "cross-referenced",
    ...
    it("should return empty array when no linked PRs", async () => {
      mockOctokit.rest.issues.listEventsForTimeline = vi.fn().mockResolvedValue({ data: [] });
      const result = await getLinkedPRs(mockOctokit, context, 1);
      globalExpect(result).toHaveLength(0);
    });
  });
  describe("countMergedPRs", () => {
    it("should count merged PRs correctly", async () => {
      ...
      mockOctokit.rest.pulls.get = vi.fn().mockResolvedValueOnce({ data: { merged: true } }).mockResolvedValueOnce({ data: { merged: false } }).mockResolvedValueOnce({ data: { merged: true } });
      const result = await countMergedPRs(mockOctokit, context, linkedPRs);
      globalExpect(result).toBe(2);
      globalExpect(mockOctokit.rest.pulls.get).toHaveBeenCalledTimes(3);
    });
  ...
  describe("hasIssueReferenceInPRs", () => {
    ...
    it("should return true when any PR contains issue reference", async () => {
      ...
      mockOctokit.rest.pulls.get = vi.fn().mockResolvedValueOnce({ data: { body: "No reference here" } }).mockResolvedValueOnce({ data: { body: "Closes #5" } });
      const result = await hasIssueReferenceInPRs(
        mockOctokit,
        context,
        5,
...
@vitest/snapshot/dist/index.js:
   ...
   *)
*/
//# sourceMappingURL=pr-analyzer.test.js.map
