...
async function classifyIssues(octokit, context, issues, options) {
  ...
  completed.sort((a3, b2) => b2.number - a3.number);
  inProgress.sort((a3, b2) => b2.number - a3.number);
  pending.sort((a3, b2) => b2.number - a3.number);
  return { completed, inProgress, pending };
}

// src/github/issue/issue-classifier.test.ts
...
describe("classifyIssues", () => {
  ...
  it("should classify issue as pending when linked PRs < minLinkedPRs", async () => {
    ...
    vi.mocked(getLinkedPRs).mockResolvedValue([]);
    const result = await classifyIssues(mockOctokit, context, issues, {
      minLinkedPRs: 1,
      minMergedPRs: 1
    });
    globalExpect(result.pending).toEqual([
      { number: 1, title: "Issue 1", user: { login: "user1" } }
    ]);
    globalExpect(result.inProgress).toEqual([]);
    globalExpect(result.completed).toEqual([]);
  });
  it("should classify issue as in-progress when merged PRs < minMergedPRs", async () => {
    const issues = [{ number: 1, title: "Issue 1", user: { login: "user1" } }];
    const mockLinkedPRs = [{ number: 10 }];
    vi.mocked(getLinkedPRs).mockResolvedValue(mockLinkedPRs);
    vi.mocked(countMergedPRs).mockResolvedValue(0);
    const result = await classifyIssues(mockOctokit, context, issues, {
      minLinkedPRs: 1,
      minMergedPRs: 1
    });
  ...
  it("should classify issue as completed when merged PRs meet threshold and has issue reference", async () => {
    ...
    const mockLinkedPRs = [{ number: 10 }];
    vi.mocked(getLinkedPRs).mockResolvedValue(mockLinkedPRs);
    vi.mocked(countMergedPRs).mockResolvedValue(1);
    vi.mocked(hasIssueReferenceInPRs).mockResolvedValue(true);
    const result = await classifyIssues(mockOctokit, context, issues, {
      minLinkedPRs: 1,
      minMergedPRs: 1
    });
  ...
  it("should classify issue as in-progress when merged PRs meet threshold but no issue reference", async () => {
    ...
    vi.mocked(getLinkedPRs).mockResolvedValue(mockLinkedPRs);
    vi.mocked(countMergedPRs).mockResolvedValue(1);
    vi.mocked(hasIssueReferenceInPRs).mockResolvedValue(false);
    const result = await classifyIssues(mockOctokit, context, issues, {
      minLinkedPRs: 1,
      minMergedPRs: 1
    });
  ...
  it("should classify multiple issues correctly", async () => {
    ...
    vi.mocked(getLinkedPRs).mockResolvedValueOnce([]).mockResolvedValueOnce([{ number: 10 }]).mockResolvedValueOnce([{ number: 11 }]);
    vi.mocked(countMergedPRs).mockResolvedValueOnce(0).mockResolvedValueOnce(1);
    vi.mocked(hasIssueReferenceInPRs).mockResolvedValueOnce(true);
    const result = await classifyIssues(mockOctokit, context, issues, {
      minLinkedPRs: 1,
      minMergedPRs: 1
    });
