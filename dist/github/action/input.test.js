...
// src/github/action/input.test.ts
describe("parseInputs", () => {
  ...
  it("should parse custom PR thresholds", () => {
    ...
    const result = parseInputs();
    globalExpect(result.minLinkedPRs).toBe(2);
    globalExpect(result.minMergedPRs).toBe(3);
  });
  it("should parse single target label", () => {
    process.env.GITHUB_TOKEN = "test-token";
    process.env.TARGET_LABELS = "auto";
    const result = parseInputs();
    globalExpect(result.targetLabels).toEqual(["auto"]);
  });
  it("should parse multiple target labels", () => {
    process.env.GITHUB_TOKEN = "test-token";
    process.env.TARGET_LABELS = "bug, enhancement, auto";
    const result = parseInputs();
    globalExpect(result.targetLabels).toEqual(["bug", "enhancement", "auto"]);
  });
  it("should filter out empty target labels", () => {
    process.env.GITHUB_TOKEN = "test-token";
    process.env.TARGET_LABELS = "bug, , auto";
    const result = parseInputs();
    globalExpect(result.targetLabels).toEqual(["bug", "auto"]);
  });
